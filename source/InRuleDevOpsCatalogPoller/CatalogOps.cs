...
namespace InRuleDevOpsCatalogPoller
{
    public static class CatalogOps
    {
        static string InRuleDevOpsServiceUri = SettingsManager.Get("InRuleDevOpsServiceUri");
        ...
        private static void SendToDevOpsServicePOST(ExpandoObject data, TraceWriter log)
        {
            try
            {
                ...
                var request = (HttpWebRequest)WebRequest.Create($"{InRuleDevOpsServiceUri}/ProcessInRuleEvent");
            ...
            {
                log.Error($"Error sending event to InRule DevOps service: {ex.Message}\r\n{ex.StackTrace}");
                WriteError($"Error sending event to InRule DevOps service: " + ex.Message);
            }
        }
